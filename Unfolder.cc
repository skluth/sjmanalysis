...
void Unfolder::unfold( Observable* obs ) const {
  ...
  vector<Double_t> correctionFactors= divideVectors( valuesHadronlevel, valuesMeasuredMC );
  DataStructure* measured= obs->getDataStructure( measuredAnalysis );
  vector<Double_t> valuesMeasured= measured->getValues();
  vector<Double_t> correctedValues= multiplyVectors( valuesMeasured, correctionFactors );
  ...
  vector<Double_t> correctedErrors= multiplyVectors( errorsMeasured, correctionFactors );
